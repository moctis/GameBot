//
//    Copyright (c) by Gajatko, none rights reserved.
//
...
namespace CoreBLL
{
    ...
    /// to brak called method. Also events for get and set accesors are provided.
    /// </summary>
    /// <typeparam name="T">Type of items.</typeparam>

    [DebuggerDisplay("Count = {Count}")]
    [Serializable()]
    public class EventyList<T> : IList<T>, System.Collections.IList
    {

        #region Delegates & events delecrations and ActionEventArgs class

        public delegate void OnSetEventHandler(EventyList<T> sender, T oldValue, T newValue);
        public delegate void BeforeSetEventHandler(EventyList<T> sender, ref T oldValue, ref T newValue, ref bool cancel);
        public delegate void OnClearEventHandler(EventyList<T> sender);
        public delegate void BeforeClearEventHandler(EventyList<T> sender, ref bool cancel);
        public delegate void EventyListAfterEventHandler(EventyList<T> sender, T item);
        public delegate void EventyListBeforeEventHandler(EventyList<T> sender, ref T item, ref bool cancel);
        public delegate void OnRemoveAtEventHandler(EventyList<T> sender, int index);
        public delegate void BeforeRemoveAtEventHandler(EventyList<T> sender, ref int index, ref bool cancel);
        public delegate void OnAddRangeEventHandler(EventyList<T> sender, IEnumerable<T> collection);
        public delegate void BeforeAddRangeEventHandler(EventyList<T> sender, IEnumerable<T> collection, ref bool cancel);
        public delegate void OnRemoveRangeEventHandler(EventyList<T> sender, int start, int count);
        public delegate void BeforeRemoveRangeEventHandler(EventyList<T> sender, ref int start, ref int count, ref bool cancel);
        public delegate void OnActionEventHandler(EventyList<T> sender, ActionEventArgs e);
        public delegate void BeforeActionEventHandler(EventyList<T> sender, ActionEventArgs e, ref bool cancel);
        ...
        public EventyList(int capacity)
        {
            if (capacity < 0) throw new ArgumentOutOfRangeException();
            arr = new T[capacity];
        }

        public EventyList(IEnumerable<T> collection)
        {
            if (collection == null)
            {
                arr = new T[0];
        ...
        public EventyList<outType> ConvertAll<outType>(Converter<T, outType> converter)
        {
            if (converter == null)
                return null;

            EventyList<outType> list = new EventyList<outType>(size);
            for (int i = 0; i < size; i++)
        ...
        public EventyList<T> GetRange(int index, int count)
        {
            if (index < 0)
            ...
            EventyList<T> list = new EventyList<T>(count);
        ...
        {
            private EventyList<T> list;
            private int index;
            private int version;
            private T current;

            internal Enumerator(EventyList<T> lst)
            {
                this.list = lst;
                index = 0;
                version = lst.ver;
